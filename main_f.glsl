...
#if (defined IS_IRIS && defined DISTANT_HORIZONS && DH_SHADOWS == 1 && DUAL_DISTORT == 1) || PIXEL_LOCKED_SHADOW_RES > 0
		//per fragment shadow pos
	uniform mat4 gbufferModelViewInverse;
	uniform mat4 shadowModelView;
#endif
#if (defined IS_IRIS && defined DISTANT_HORIZONS && DH_SHADOWS > 0 && LONG_SUNSET_SHADOWS == 1) ||  PIXEL_LOCKED_SHADOW_RES > 0
...
void main() {
	...
	#if PIXEL_LOCKED_SHADOW_RES > 0
	vec4 shadow_pos =vec4( vec4(gbufferModelViewInverse*vec4(viewPos.xyz,1.)).xyz ,1.);
		//shadow_pos.xyz-=gbufferModelViewInverse[3].xyz;
		vec3 shadow_n=vec4(gbufferModelViewInverse*vec4(normals_face.xyz-gbufferModelViewInverse[3].xyz,1.)).xyz;
		float dot_face_sun = dot(normals_face.xyz,normalize(shadowLightPosition));

		float sm_bias = 1./PIXEL_LOCKED_SHADOW_RES +SHADOW_BIAS_PL*(1.-(abs(dot_face_sun)));
		...
		shadowPos=shadowPosv;
			...
			#if PIXEL_LOCKED_SHADOW_RES > 0
				playerPos= floor(playerPos*PIXEL_LOCKED_SHADOW_RES)/PIXEL_LOCKED_SHADOW_RES;
			#endif
	...
	float face_shading = clamp(shadowPos.w,0.,1.);
		...
		vec4 color= glcolor;
			...
			#else
				//color.rgb*=glcolor.rgb; //*glcolor.a; // ao is in alpha for terrain
				vec4 color = texture2D(texture, texcoord) * vec4(glcolor.rgb,1.);
			#endif
			#if PBR > 0
	...
	vec2 lm = lmcoord;
		...
		float sky_lighting = clamp((dot(n_sky_dir,normals_pixel.xyz)+1.)*.5, NON_DIRECTIONAL_AMBIENT_SKY_LIGHT ,1.);//fix?
		...
		#else
			float sun_lighting=  face_shading
			
			; //sunlighting wo normals
		#endif
		...
		#if FLASH_LIGHT == 2
			float f_dist = clamp( 1.-distance( vec3(0.),viewPos.xyz )/ (HAND_HELD_TORCH_RANGE 
				#if FLASH_LIGHT >= 1
					* flashlight_distance_mult
				#endif
			) ,0.,1.);
			float radial_fl = (1. -2.* 
				distance(gl_FragCoord.xy,.5*vec2(viewWidth,viewHeight))
					/(viewHeight*2.*FLASH_LIGHT_WIDTH
					*(.25+.75*(1.-f_dist))));
				...
				vec2 f_offset = viewWidth>viewHeight? 
				vec2((viewWidth-viewHeight)*.5,0.) :
				vec2(0.,(viewHeight-viewWidth)*.5);
			vec2 flashlight_uv =  clamp( 
			(
				(gl_FragCoord.xy -f_offset)
				/min(viewWidth,viewHeight) -.5)
				/(FLASH_LIGHT_WIDTH ) 
					+.5 ,0.,1.);
			#else
				float f_dist =  clamp(1.-distance( vec3(0.),viewPos.xyz )/ (HAND_HELD_TORCH_RANGE 
			...
			(
				(gl_FragCoord.xy -f_offset)
				/min(viewWidth,viewHeight) -.5)
				/(FLASH_LIGHT_WIDTH*(pow( 1.-f_dist,FLASHLIGHT_CURVATURE) +FLASHLIGHT_MIN) ) 
					+.5 ,0.,1.);
			#endif
			
	
			vec3 flashlight = texture2D(noisetex,flashlight_uv).rgb * (f_dist)*FLASH_LIGHT_BRIGHTNESS*2. ;
		#endif
		...
		#if FLASH_LIGHT < 2
			clamp( 1.-distance( vec3(0.),viewPos.xyz )/ (HAND_HELD_TORCH_RANGE 
			#if FLASH_LIGHT >= 1
				* flashlight_distance_mult
			#endif
				...
				* .7*((f_dist+radial_fl*f_dist)+pow(abs(sin(3.+radial_fl*8.*f_dist) ),2.-radial_fl) ) *FLASH_LIGHT_BRIGHTNESS
				:1.)
			#else
				f_dist		
			*clamp( radial_fl *FLASH_LIGHT_CRISPNESS,0.,1.) 
		...
		#if IS_THE_NETHER == 1 && NETHER_TORCH_lIGHT_PULSE == 1
	...
	color.rgb*specular_pixel.a*PBR_EMMISSIVE_STRENGTH+
		...
		color.rgb* max(
				...
				#if PULSED_LIGHT_MATCH_FOG == 1
					fogColor
				#else
					vec3(1.,0.9,0.7)
				#endif
				-vec3(1.,2.5,3.)*world_pos.y/100.
